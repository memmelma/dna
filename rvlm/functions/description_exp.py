...
# Short JSON (object lists for grounding). Small cap + greedy decoding in call_api so the
# model cannot emit dozens of repeated ```json blocks before EOS (see qwen greedy flag).
_GROUNDING_OBJECT_MAX_NEW_TOKENS = 1024
_GROUNDING_JSON_RULES = (
    "\n\nRespond once only: a single JSON value (array or object). "
    "No markdown code fences. Do not repeat or rephrase the answer."
)


def _first_json_value(text: str):
    """Parse the first JSON object or array in the string (skip preamble / extra fences)."""
    ...
    for i, c in enumerate(s):
        if c in "[{":
            try:
                ...
                continue
    raise ValueError(f"No JSON found in model output (prefix): {s[:800]!r}")


def response_to_json(response) -> dict:
    """Parse first JSON object from model output; log the parsed payload only (not full decode)."""
    response_json = _first_json_value(response.text)
    try:
        to_show = response_json[0]
    except KeyError:
        to_show = response_json
    print("[grounding objects]", json.dumps(to_show, ensure_ascii=False)[:1500])
    try:
...
async def get_description_from_video_grounded(
    ...
    if cache_key in _grounding_cache:
        ...
        prompt = prompt.replace("{task}", task) + _GROUNDING_JSON_RULES
        res = await call_api(
            prompt,
            video_input=video,
            ...
            max_new_tokens=_GROUNDING_OBJECT_MAX_NEW_TOKENS,
            greedy=True,
        )
    ...
    return _parse_descriptions_payload(_first_json_value(res.text))
...
async def get_description_from_video(
    ...
    #     <role>
    #     You are an expert in scene understanding. Provide highly detailed descriptions including the robot and state of all objects in the video at each timestep.
    #     Describe robot motion and distance between robot and objects.
    #     Without looking at the rest of the video, describe the scene in the first and last frame to ground the descriptions.
    #     </role>
...
async def get_description_from_all_frames_grounded(
    ...
    if cache_key in _grounding_cache:
        ...
        prompt_g = prompt_g.replace("{task}", task) + _GROUNDING_JSON_RULES
...
async def get_description_from_single_frames(
    ...
    async def one_frame(i: int) -> str:
        ...
        data = _first_json_value(res.text)

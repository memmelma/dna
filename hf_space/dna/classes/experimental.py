...
def _extract_completion_state(text):
    ...
    ``text`` is the raw response string for most reasonings, or a ``json.dumps``
    of several response texts for the distributional reasoning.
    """
    if not text:
        return None
    try:
        ...
        return None
    # Distributional path: a list of raw response-text strings.
...
class Experimental:
    ...
    async def _compute_progress_once_async(self, frames_array: np.ndarray, task_description: str = "") -> List[Optional[float]]:
        """
        ...
        if self.verbose >= 2 and self.root_dir is not None:
    ...
    async def compute_progress_async(self, frames_array: np.ndarray, task_description: str = "", *, n_samples: int = 1) -> List[Optional[float]]:
        """
        ...
        :return: (N,) array of per-frame progress values in [0, 1], averaged over n_samples
        """
        n_samples = max(1, int(n_samples))
        results = await asyncio.gather(
            *[self._compute_progress_once_async(frames_array, task_description) for _ in range(n_samples)]
        )
        if n_samples == 1:
            return results[0]
        return np.mean(results, axis=0)

    def compute_progress(self, frames_array: np.ndarray, task_description: str = "", *, n_samples: int = 1) -> List[Optional[float]]:
        """
        ...
        """
        return asyncio.run(self.compute_progress_async(frames_array, task_description, n_samples=n_samples))